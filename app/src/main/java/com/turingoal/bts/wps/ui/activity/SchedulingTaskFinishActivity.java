...
import com.turingoal.bts.wps.ui.adapter.DispatchTaskItemFinishAdapter;
 ...
 * 结束任务
 */
@Route(path = TgArouterPaths.SCHEDULING_TASK_FINISH)
public class SchedulingTaskFinishActivity extends TgBaseActivity {
    ...
    DispatchTaskItemFinishAdapter mAdapter = new DispatchTaskItemFinishAdapter(); // adapter
    @Autowired
    SchedulingTask schedulingTask; // ARouter自动解析参数对象

    @Override
    protected int getLayoutId() {
        return R.layout.app_activity_scheduling_task_finish;
    }
    ...
    public void onClick() {
        TgDialogUtil.showDialog(this, "确定要结束任务作业吗？", new MaterialDialog.SingleButtonCallback() {
            @Override
            public void onClick(final @NonNull MaterialDialog materialDialog, final @NonNull DialogAction dialogAction) {
                TgApplication.getRetrofit().create(TaskService.class)
                        .finish(schedulingTask.getId())
                        .enqueue(new TgRetrofitCallback<Object>(SchedulingTaskFinishActivity.this, true, true) {
                            @Override
                            public void successHandler(Object obj) {
                                EventBus.getDefault().post("UpdateSuccess");
                                TgToastUtil.showLong("结束任务作业成功"); // 弹出信息
