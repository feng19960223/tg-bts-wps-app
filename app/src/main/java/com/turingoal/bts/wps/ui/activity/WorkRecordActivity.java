...
import com.turingoal.bts.wps.ui.adapter.WorkRecordListAdapter;
 ...
 * 作业记录
 */
@Route(path = TgArouterPaths.WORKRECORD)
public class WorkRecordActivity extends TgBaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private WorkRecordListAdapter mAdapter = new WorkRecordListAdapter(); // adapter
    private Date currentDate; // 日期

    @Override
    protected int getLayoutId() {
        return R.layout.app_activity_work_record;
    }

    @Override
    protected void initialized() {
        initToolbar(R.id.tv_title, R.id.iv_back, "作业记录"); // 顶部工具条
    ...
    private void getData(Date date) {
        ...
        TgApplication.getRetrofit().create(TaskService.class)
                .findFinishedByDate(TgDateUtil.date2String(currentDate), TgApplication.getTgUserPreferences().getUserId())
    ...
    public void onClick(final View v) {
        switch (v.getId()) {
            ...
            case R.id.query_time_right:
                ...
                break;
            case R.id.iv_right:
                TgSystemHelper.handleIntent(TgArouterPaths.WORKRECORD_ALL); // 全部作业页面
                break;
            default:
                break;
        }
