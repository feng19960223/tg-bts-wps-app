 ...
 * [故障处理开始]
 */
@Route(path = TgArouterPaths.BREAKDOWN_HANDLING_START)
public class BreakdownRecordHandlingStartActivity extends TgBaseActivity {
    ...
    protected int getLayoutId() {
        return R.layout.app_activity_breakdown_handling_start;
    }

    @Override
    protected void initialized() {
        initToolbar(R.id.tv_title, R.id.iv_back, "开始故障检修");
                    ...
                    public void successHandler(BreakdownRecordItem breakdownRecordItem) {
                        if (breakdownRecordItem.getFilePaths() != null) {
                            if (breakdownRecordItem.getFilePaths().size() > 0) {
                                recyclerView.setLayoutManager(new GridLayoutManager(BreakdownRecordHandlingStartActivity.this, SPAN_COUNT));
    ...
    public void breakdownHandlingStart() {
        TgApplication.getRetrofit().create(BreakdownRecordItemService.class)
                .start(breakdownRecordItem.getId())
                .enqueue(new TgRetrofitCallback<Object>(this, true, true) {
                    @Override
                    public void successHandler(final Object obj) {
                        TgToastUtil.showLong("开始故障检修作业成功！");
