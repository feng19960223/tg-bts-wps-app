 ...
 * 快速作业，完成作业
 */
@Route(path = TgArouterPaths.FASTWORK_LEVEL1_FINISH)
public class FastWorkLevel1FinishActivity extends TgBaseActivity {
    ...
    protected int getLayoutId() {
        return R.layout.app_activity_fast_work_finish;
    }

    @Override
    protected void initialized() {
        initToolbar(R.id.tv_title, R.id.iv_back, "完成作业"); // 顶部工具条
        ...
        tvStatus.setText("【有电作业任务】");
        tvMessage.setVisibility(View.VISIBLE);
        tvMessage.setText("请等待检修工长确认完成有电作业任务！");
        if (ConstantStatus4Work.FINISHED_WAIT.equals(dispatchTaskItem.getStatus()) || TgAppConfig.MODE_FOREMAN_AUTO) { // 任务完成
            btnSubmit.setEnabled(true); // 启用提交按钮
            btnSubmit.setBackground(getDrawable(R.drawable.common_btn_green_style));
    ...
    public void onClick(final View v) {
        switch (v.getId()) {
            case R.id.btn_submit:
                TgDialogUtil.showDialog(this, "确定要完成全部作业吗？", new MaterialDialog.SingleButtonCallback() {
                    @Override
    ...
    private void submitRequest() {
        TgApplication.getRetrofit().create(FastWorkService.class)
                .finish(dispatchTaskItem.getId())
                .enqueue(new TgRetrofitCallback<Object>(this, true, true) {
                    @Override
                    public void successHandler(Object object) {
                        EventBus.getDefault().post("UpdateSuccess");
                        TgToastUtil.showLong("完成作业！"); // 弹出信息
                        defaultFinish(); // 关闭当前页面，刷新作业列表
                    }
                });
    }
}