...
public class TgChangePasswordActivity extends TgBaseActivity {
    ...
    private void updateUserPass() {
        ...
        if (!validateResult.isSuccess()) { // 验证失败
            ...
            submitRequest(passwordOld, passwordNew); // 提交请求
        }
    }
     ...
     * 请求
     */
    private void submitRequest(final String oldPass, final String userPass) {
        String oldPassStr = TgEncryptUtil.encryptMD5ToString("tg_pass" + oldPass).toLowerCase();
        ...
        TgApplication.getRetrofit().create(UserService.class)
                .changePass(TgApplication.getTgUserPreferences().getUserId(), oldPassStr, userPassStr)
                .enqueue(new TgRetrofitCallback<Object>(this, true, true) {
                    @Override
                    public void successHandler(Object obj) {
                        TgToastUtil.showLong("密码修改成功，请重新登录！"); // 修改成功，请重新登录
                        TgSystemHelper.logoff(TgChangePasswordActivity.this); // 注销并跳转到登录页面
                    }
                });
    }
}
