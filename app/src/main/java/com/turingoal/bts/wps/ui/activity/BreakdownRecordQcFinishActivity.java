 ...
 * [故障质检结束]
 */
@Route(path = TgArouterPaths.BREAKDOWN_QC_FINISH)
public class BreakdownRecordQcFinishActivity extends TgBaseActivity {
    ...
    @BindView(R.id.etQcDesc)
    EditText etQcDesc; // 质检描述
    @BindView(R.id.tvQcDescCount)
    TextView tvQcDescCount; // etQcDesc字数
    ...
    protected int getLayoutId() {
        return R.layout.app_activity_breakdown_qc_finish;
    }

    @Override
    protected void initialized() {
        initToolbar(R.id.tv_title, R.id.iv_back, "结束故障质检");
        ...
        etQcDesc.addTextChangedListener(countWatcher);
                    ...
                    public void successHandler(BreakdownRecordItem breakdownRecordItem) {
                        if (breakdownRecordItem.getFilePaths() != null) {
                            if (breakdownRecordItem.getFilePaths().size() > 0) {
                                recyclerView.setLayoutManager(new GridLayoutManager(BreakdownRecordQcFinishActivity.this, SPAN_COUNT));
    ...
    private TextWatcher countWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(final Editable editable) {
            tvQcDescCount.setText("" + editable.toString().trim().length() + "/140"); // 文字计数 140个
        }
    };

    @OnClick(R.id.btnFinish)
    public void breakdownQcFinish() {
        String qcDesc = etQcDesc.getText().toString().trim();
        if (TgStringUtil.isBlank(qcDesc)) {
            TgToastUtil.showLong("请填写质检描述！");
            return;
        }
        TgApplication.getRetrofit().create(BreakdownRecordItemService.class)
                .qcFinish(breakdownRecordItem.getId(), qcDesc)
                .enqueue(new TgRetrofitCallback<Object>(this, true, true) {
                    @Override
                    public void successHandler(final Object obj) {
                        TgToastUtil.showLong("结束故障质检作业成功！");
